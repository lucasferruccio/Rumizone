...
class rumizone(CTk):
    ...
    def menu(self):
        ...
        self.btmonitoramento = CTkButton(master=self.root, image=btn_monitoramento,text="", fg_color=verde_fundo, width= 100,height= 70,command=self.monitoramento_comportamento)
        self.btmonitoramento.place(x=650,y=100)

        #Botão para o Frame de Registros
        self.btregistros = CTkButton(master=self.root, image=btn_registros,text="", fg_color=verde_fundo, width= 100,height= 7, command=self.relatorios)
        self.btregistros.place(x=300,y=380)

    #Minha conta:
    #Botão para o Frame de Análise Comportamental
        self.btrendimento = CTkButton(master=self.root, image=btn_animais,text="", fg_color=verde_fundo, width= 100,height= 70, command=self.animais)
    ...
    def menu_conta(self):
        ...
        self.frame_info_conta = CTkFrame(master=self.menu_conta_Frame, width=900, height=400, fg_color=verde, border_width=2, border_color=verde)
        ...
        #Cargo
        self.texto_cargo = CTkLabel(master=self.frame_info_conta, text="Cargo: " + informacoes[1],font=custom_font_label)
        self.texto_cargo.place(x=20,y=91)

        #cpf
        self.texto_cpf = CTkLabel(master=self.frame_info_conta, text="CPF: " + str(informacoes [2]),font=custom_font_label)
        self.texto_cpf.place(x=20,y=152)

        #Email
        self.texto_email = CTkLabel(master=self.frame_info_conta, text="Email: " + str(informacoes[3]),font=custom_font_label)
        self.texto_email.place(x=20,y=213)

        #Endereço
        self.texto_endereco = CTkLabel(master=self.frame_info_conta, text="Endereço: " + str(informacoes[4]),font=custom_font_label)
        self.texto_endereco.place(x=20,y=274)

        #Login
        self.texto_login = CTkLabel(master=self.frame_info_conta, text="Login: " + str(informacoes[5]),font=custom_font_label)
        self.texto_login.place(x=20,y=335)

        #Texto aviso
        self.texto_aviso = CTkLabel(master=self.menu_conta_Frame, text="Para entrar em contato com o suporte ligue para: 2101-1111", font=aviso)
    ...
    def atualizar_conta_frame(self):
        ...
        #Nome
        self.texto_nome = CTkLabel(master=self.frame_info_conta, text="Nome: ", font=custom_font_label)
        self.texto_nome.place(x=20,y=31)
        self.entry_nome = CTkEntry(master=self.frame_info_conta, placeholder_text=informacoes[0])
        self.entry_nome.place(x=100,y=31)
        
        #Cargo
        self.texto_cargo = CTkLabel(master=self.frame_info_conta, text="Cargo: ", font=custom_font_label)
        self.texto_cargo.place(x=20,y=91)
        self.entry_cargo = CTkEntry(master=self.frame_info_conta, placeholder_text=informacoes[1])
        self.entry_cargo.place(x=100,y=91)

        #cpf
        self.texto_cpf = CTkLabel(master=self.frame_info_conta, text="CPF: " + informacoes[2], font=custom_font_label)
        self.texto_cpf.place(x=20,y=152)

        #Email
        self.texto_email = CTkLabel(master=self.frame_info_conta, text="Email: ", font=custom_font_label)
        self.texto_email.place(x=20,y=213)
        self.entry_email = CTkEntry(master=self.frame_info_conta, placeholder_text=informacoes[3])
        self.entry_email.place(x=90,y=213)

        #Endereço
        self.texto_endereco = CTkLabel(master=self.frame_info_conta, text="Endereço: ", font=custom_font_label)
        self.texto_endereco.place(x=20,y=274)
        self.entry_endereco = CTkEntry(master=self.frame_info_conta, placeholder_text=informacoes[4])
        self.entry_endereco.place(x=120,y=274)

        #Login
        self.texto_login = CTkLabel(master=self.frame_info_conta, text="Login: " + informacoes[5],font=custom_font_label)
        self.texto_login.place(x=20,y=335)

        #Botão para voltar ao menu
        self.btn_voltar_menu = CTkButton(master=self.root, text="Voltar", fg_color="#123529", width= 75,height= 25, command=self.menu_conta)
        self.btn_voltar_menu.place(x=10,y=10)
    ...
    def atualizar_conta(self):
        ...
        valores = {"nome": nome, "cargo": cargo, "email": email, "endereco": endereco}
        resposta = atualizar_dados_conta(valores, id[0])
        if resposta == True:
            messagebox.showinfo("Atualizar Conta", "Conta atualizada com sucesso!")
            self.menu_conta()
        else:
    ...
    def informacoes_vacas(self, info):
        ...
        self.manejo_1 = CTkLabel(master=self.frame_info,width=300, text= "Data de Manejo:", height=50, font=custom_font_label)
    ...
    def cadastrar_animal(self):
        ...
        if quantidade[0][0] < 12:
            ...
            messagebox.showerror("Cadastro Animal", "Número máximo de aniamis atingido")
            self.animais()
    ...
    def monitoramento_comportamento(self):
        ...
        #Informações do Frame dos animais
        self.monitoramento_Frame = CTkFrame(master=self.root, width=1200, height=700, fg_color=verde_fundo)
    ...
    def relatorios(self):
        ...
        #Informações do Frame dos animais
        self.relatorios_frames = CTkFrame(master=self.root, width=1200, height=700, fg_color=verde_fundo)
        ...
        self.btn_cadastrar_relatorio= CTkButton(master=self.root, text="Adicionar", fg_color="#123529", width= 75,height= 25, command=self.criar_relatorio_frame)
    ...
    def relatorio_pagina(self, info, nome):
            ...
            self.adicionar_relatorio_frames = CTkFrame(master=self.root, width=1200, height=700, fg_color=verde_fundo)
            self.adicionar_relatorio_frames.grid(row=0,column=0)
            custom_font_label = CTkFont(family="Roboto", size=12, weight="bold")
            
            #Frame que vai conter as informações
            self.frame_info_relatorio = CTkFrame(master=self.adicionar_relatorio_frames, width=600, height=600, fg_color=verde_botao_fundo, border_width=2, border_color=verde_botao_fundo)
            ...
            self.texto_1 = CTkLabel(master=self.frame_info_relatorio,width=100, text= "Relatório:", height=50, font=custom_font_label, anchor="w")
            self.texto_1.place(x=50,y=70)
            self.texto_2 = CTkTextbox(master=self.frame_info_relatorio, width=500, height=300, fg_color="#1B4D3B")
            ...
            self.info_relatorio = CTkLabel(master=self.frame_info_relatorio, text=data_formatada + ", " + nome, fg_color="#1B4D3B", width=200, anchor="w", padx=10, corner_radius=5)
            self.info_relatorio.place(x=50, y = 450)

            #Botão para voltar paro  a pagina de relatorios

            self.btn_voltar_relatorio = CTkButton(master=self.root, text="Voltar", fg_color="#123529", width= 75,height= 25, command=self.voltar_relatorio)
    ...
    def atualizar_relatorio_frame(self, info, nome):
        #Detruir frame anterior
            ...
            self.texto_1 = CTkLabel(master=self.frame_info_relatorio,width=100, text= "Relatório:", height=50, font=custom_font_label, anchor="w")
            self.texto_1.place(x=50,y=70)
            self.texto_2 = CTkTextbox(master=self.frame_info_relatorio, width=500, height=300, fg_color=branco, text_color="#000000")
            self.texto_2.place(x=50, y=120)
            self.texto_2.insert("0.0", info[3])

            #Botão para cancelar
            self.btn_cancelar_relatorio = CTkButton(master=self.frame_info_relatorio, text="Caneclar", fg_color="#123529", width= 100,height= 50,
    ...
    def relatorio_deletar(self, id):
        ...
        if resposta == True:
            messagebox.showinfo("Exclusão Registro", "Exclusão completa com sucesso!")
            self.relatorios()
        else:
    ...
    def atualizar_relatorio_dados(self, info):
        ...
        if resposta == True:
            messagebox.showinfo("Atualizar Registro", "Atualização completa com sucesso!")
            self.relatorios()
        else:
    ...
    def relatorio_criar(self):
        ...
        resposta = adicionar_relatorio(id[0], titulo, relatorio, data_formatada)
        if resposta == True:
...
btn_conta = CTkImage(Image.open(pathFinal + "/caixa_conta.png"), size=(200,200))
btn_animais = CTkImage(Image.open(pathFinal + "/caixa_animais.png"), size=(200,200))
btn_monitoramento = CTkImage(Image.open(pathFinal + "/caixa_monitoramento.png"), size=(200,200))
btn_registros = CTkImage(Image.open(pathFinal + "/caixa_relatorio.png"), size=(200,200))
