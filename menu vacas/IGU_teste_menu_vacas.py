...
class telaconta:
    def __init__(self,telac):
        ...
        self.btv.grid(x=1,y=0)
    def voltar(self):
        telamenu(tela)
class telaboi:
    def __init__(self,telab):
        self.telavaca = CTkToplevel(telab)
        self.telavaca.title("menu vacas")
        self.telavaca.geometry("1200x700")
        self.frame = CTkFrame(self.telavaca,width=1200, height= 700,fg_color = marrom)
        self.frame.grid(row=1,column=0)
        self.btv = CTkButton(self.telavaca,text="voltar",width=0, command= self.voltar)
        self.btv.grid(row = 0, column = 0)
        btsvacas(self.frame)
